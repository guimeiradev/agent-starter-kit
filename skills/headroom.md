...
3. **Upstream chaining (optional).** Headroom's own upstream defaults to `https://api.anthropic.com`. It can instead point at a multi-provider gateway (e.g. OmniRoute) via `--anthropic-api-url <gateway-url>/v1`, so compression happens before multi-provider routing/fallback. Only do this once the target gateway is fully configured (logged in, provider API keys set) — verify with a direct health check against the gateway first, don't assume.
